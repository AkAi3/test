...
inappropriate_words = ['so', 'rrr', 'rr']  # Add the words you want to censor
...
def process_audio(audio_path):
    ...
    chunk_length_ms = 1500 # 2-second chunks for better context
    chunks = make_chunks(audio, chunk_length_ms)  # Chunk audio into 2-second segments
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            # Adjust for ambient noise based on the current chunk
            ...
            if any(word in text.lower() for word in inappropriate_words):
                ...
                subtitles.append((censored_text, True, i * chunk_length_ms))  # Store subtitle, censorship flag, and timestamp
            else:
                subtitles.append((censored_text, False, i * chunk_length_ms))  # Store subtitle and timestamp

            final_audio += chunk

        except sr.UnknownValueError:
            print(f"Chunk {i}: Could not understand audio")
            subtitles.append(("Could not understand audio", False, i * chunk_length_ms))
            final_audio += chunk
        except sr.RequestError as e:
            print(f"Chunk {i}: Could not request results; {e}")
            subtitles.append((f"Could not request results; {e}", False, i * chunk_length_ms))
            final_audio += chunk
